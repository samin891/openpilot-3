...
class LongControl():
  ...
  def update(self, active, CS, CP, long_plan, accel_limits, radarState):
    ...
    # Intention is to stop, switch to a different brake control until we stop
      ...
      factor = 1
      if long_plan.hasLead:
        factor = interp(dRel,[2.0,3.6], [2.0,1.0])
    ...
    if CP.sccBus != 0 and self.long_log:
      str_log3 = 'BS={:1.0f}/{:1.0f}/{:1.0f}  LS={:s}  LP={:s}  ACCEL={:+04.2f}/{:+04.2f}  GS={}  RD={:04.1f}'.format(CP.mdpsBus, CP.sccBus, CP.fcaBus, self.long_stat, self.long_plan_source, final_accel, output_accel, int(CS.gasPressed), CS.radarDistance)
