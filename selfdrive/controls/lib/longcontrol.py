...
class LongControl():
  ...
  def update(self, active, CS, CP, long_plan, accel_limits, radarState):
    ...
    if CP.sccBus != 0 and self.long_log:
      str_log3 = 'BS={:1.0f}/{:1.0f}  GS={:01.2f}/{:01.2f}  BK={:01.2f}/{:01.2f}  GB={:+04.2f}  GS={}  RD={:04.1f}  TG={:04.2f}/{:+04.2f}'.format(CP.mdpsBus, CP.sccBus, final_gas, gas_max, abs(final_brake), abs(brake_max), output_gb, int(CS.gasPressed), CS.radarDistance, v_target, a_target)
