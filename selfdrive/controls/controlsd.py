...
class Controls:
  ...
  def state_transition(self, CS):
    ...
    if not self.CP.pcmCruise:
      ...
      elif CS.cruiseButtons == Buttons.RES_ACCEL and CS.cruiseState.modeSel != 0 and t_speed <= self.v_cruise_kph_last <= round(CS.vEgo*m_unit):
  ...
  def state_control(self, CS):
    ...
    if not self.joystick_mode:
      ...
      actuators.accel = self.LoC.update(self.active and (CS.cruiseAccStatus or vSetDis > 30), CS, self.CP, long_plan, pid_accel_limits, self.sm['radarState'])
