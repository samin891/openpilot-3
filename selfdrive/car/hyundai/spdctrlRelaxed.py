    ...
    def update_lead(self, sm, CS, dRel, yRel, vRel):
        ...
        if CS.driverAcc_time and not self.map_decel_only:
            ...
            #    lead_wait_cmd, lead_set_speed = self.get_tm_speed( CS, 15, 5)
                ...
                lead_wait_cmd, lead_set_speed = self.get_tm_speed( CS, 8, 1)
            elif CS.clu_Vanz > 80 and lead_objspd < -1 and (int(CS.clu_Vanz)-1) <= int(CS.VSetDis) and int(CS.clu_Vanz) >= dRel*1.6 and 1 < dRel < 149:
                self.seq_step_debug = 22
                lead_wait_cmd, lead_set_speed = self.get_tm_speed( CS, max(8, 50+(lead_objspd*2)), -1)
            elif CS.clu_Vanz > 60 and lead_objspd < -2 and (int(CS.clu_Vanz)-1) <= int(CS.VSetDis) and int(CS.clu_Vanz) >= dRel*1.8 and 1 < dRel < 149:
                self.seq_step_debug = 23
                lead_wait_cmd, lead_set_speed = self.get_tm_speed( CS, max(8, 50+(lead_objspd*2)), -1)
            elif CS.clu_Vanz > 40 and lead_objspd < -2 and (int(CS.clu_Vanz)-1) <= int(CS.VSetDis) and int(CS.clu_Vanz) >= dRel*2.1 and 1 < dRel < 149:
                ...
                lead_wait_cmd, lead_set_speed = self.get_tm_speed( CS, 8, -5)
            elif lead_objspd <= 0 and int(CS.clu_Vanz)+4 <= int(CS.VSetDis) and int(CS.clu_Vanz) > 40 and 1 < dRel < 149:
