...
class CarController():
  def __init__(self, dbc_name, CP, VM):
    ...
    self.opkr_turnsteeringdisable = False
    self.steer_wind_down_enabled = False
    self.opkr_maxanglelimit = 120
  ...
  def update(self, enabled, CS, frame, actuators, pcm_cancel_cmd, visual_alert,
    ...
    self.dRel = int(plan.dRel) #EON Lead
    self.vRel = int(plan.vRel * 3.6 + 0.5) #EON Lead

    self.vCruiseSet = sm['lateralPlan'].vCruiseSet
    
    # Steering Torque
    if 0 <= self.driver_steering_torque_above_timer < 100:
      new_steer = int(round(actuators.steer * self.p.STEER_MAX * (self.driver_steering_torque_above_timer / 100)))
    else:
    ...
    can_sends.append(create_lkas11(self.packer, frame, self.car_fingerprint, apply_steer, lkas_active,
                                   ...
                                   left_lane_warning, right_lane_warning, 0, self.steer_wind_down_enabled))

    if CS.CP.mdpsBus: # send lkas11 bus 1 if mdps is bus 1
      can_sends.append(create_lkas11(self.packer, frame, self.car_fingerprint, apply_steer, lkas_active,
                                   self.steer_wind_down, CS.lkas11, sys_warning, sys_state, enabled, left_lane, right_lane,
                                   left_lane_warning, right_lane_warning, 1, self.steer_wind_down_enabled))
    ...
    # reset lead distnce after the car starts moving
      ...
      if is_sc_run:
        can_sends.append(create_clu11(self.packer, self.resume_cnt, CS.clu11, self.SC.btn_type, self.SC.sc_clu_speed, CS.CP.sccBus))
        self.resume_cnt += 1
      else:
        self.resume_cnt = 0
    ...
    if self.cancel_counter == 0 and not CS.acc_active and not CS.out.brakeLights and int(CS.VSetDis) > t_speed and \
     (CS.lead_distance < 149 or int(CS.clu_Vanz) > t_speed) and int(CS.clu_Vanz) >= 3 and self.auto_res_timer <= 0:
      can_sends.append(create_clu11(self.packer, frame, CS.clu11, Buttons.RES_ACCEL, clu11_speed, CS.CP.sccBus))  # auto res
      if self.auto_res_timer <= 0:
        self.auto_res_timer = randint(10, 15)
    elif self.auto_res_timer > 0:
